"""
ViraWatch - FastAPI Backend (V5.0)
===================================
...
def load_model_artifacts():
    """Load all models, scalers, and configs."""
    models = {}
...
def generate_forecast(state, epi_week, year, probability, is_endemic):
    ...
    # Decay or increase based on current risk
    for i in range(1, 5):
        week = epi_week + i
        year_adj = year
        if week > 52:
        ...
        # Simple forecast model: probability changes based on trend
        if probability >= 0.50:
            # High risk: slight decay
            change = -0.02 * i
        else:
            # Low risk: slight increase
            change = 0.02 * i
        
        # Add seasonality effect
        if week <= 12 or week >= 45:
            seasonality = 0.03  # Dry season increases risk
        elif 21 <= week <= 40:
            seasonality = -0.02  # Rainy season decreases risk
        else:
            seasonality = 0.01
...
def assess_input_quality(state, epi_week, year, rainfall_mm, temp_c, recent_cases, endemic_flag):
    ...
    Returns: score (0-100), level, issues, and justification.
    """
    issues = []
    score = 100
    
    # Check for default/estimated values
    if recent_cases == 0:
...
def assess_prediction_confidence(probability, risk_tier):
    """
    Assess how confident the model is about this specific prediction.
    Based on probability distance from decision threshold.
    """
    # Distance from threshold (0.5)
    distance = abs(probability - 0.5)
    normalized = min(1, distance * 2)  # Scale to 0-1
    ...
    # Calculate closeness to decision threshold
...
def calculate_agreement(rf_prob, xgb_prob, rf_tier, xgb_tier):
    ...
    Does NOT create a new probability.
    """
...
def assess_prediction_reliability(input_quality, prediction_confidence, agreement, primary_result):
    """
    Combine input quality, prediction confidence, and agreement into a reliability score.
    """
    ...
    # Factor in agreement if available
    if agreement:
...
def generate_overall_assessment(primary_result, prediction_confidence, input_quality, agreement, reliability):
    """
    Generate final overall assessment of the prediction.
    """
    # Collect evidence
    evidence = []
...
def get_primary_model():
    """Determine primary model based on validation performance."""
...
@app.get("/model-info")
def model_info():
    """Return model metadata including comparison."""
    rf_path = os.path.join(MODELS_DIR, 'model_config.json')
    xgb_path = os.path.join(MODELS_DIR, 'xgboost_config.json')
...
def predict(
    ...
    """Predict outbreak risk using specified model."""
    ...
    if not model_data or not model_data.get('loaded'):
        raise HTTPException(
            status_code=503, 
            detail=f"Model '{model}' not loaded. Please train it first."
        )
...
def predict_comparison(
    ...
    Returns:
    - Primary model prediction (best performing)
    ...
    # 2. Determine primary model
    primary_model_name = get_primary_model()
    primary_result = rf_result if primary_model_name == "Random Forest" else xgb_result
    comparison_result = xgb_result if primary_model_name == "Random Forest" else rf_result
    comparison_name = "XGBoost" if primary_model_name == "Random Forest" else "Random Forest"
    ...
    if primary_result and not isinstance(primary_result, dict):
        ...
        prediction_confidence = {"level": "Unknown", "score": 0}
    
    # 5. Agreement Analysis
    agreement = None
    if rf_result and xgb_result and not isinstance(rf_result, dict) and not isinstance(xgb_result, dict):
...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:5173",
        "http://127.0.0.1:5173",
        # Vercel preview URLs (will be generated)
        ...
        # API subdomain (optional, set up later)
